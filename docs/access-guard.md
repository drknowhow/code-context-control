...
| c3_delegate | `file_path` packed through the read verdict (denied raises, masked refuses) for every backend; claude runs tool-less, or as `scout=true` with `Read()` denies derived from deny/mask rules plus this guard as its only hook, or in write mode (`write_paths`) with `Edit()` allows for the write set, `Read()`/`Edit()` denies from deny/read_only/confirm/mask rules, and this guard in worker mode (no grants, no filed requests, write set, `.git/`/`.c3/`, locks; docs/delegate-write.md); pins codex `--sandbox read-only` when rules exist; gemini and grok write mode behind user opt-in |
