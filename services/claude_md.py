...
8. **LOG**: `c3_session(action='log')` for decisions. `c3_session(action='snapshot')` before /clear. Past sessions: `c3_session(action='note', data=<done>, reasoning=<next steps>)` before you stop; `c3_session(action='stale', target=<id>, reasoning=<why>)` when your work supersedes, finishes or abandons an earlier session (`list` shows them)
