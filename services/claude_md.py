...
11.6. **CONFIRM HOLDS** (v2.97.0+): a refusal tagged `[c3-access:confirm]` is a PAUSE, not a block — the user set that path to "ask me first". C3 already filed the confirmation request for you (the S8 text names its id); `c3_override(action='wait', request_id='...')` blocks up to 180s for the decision, then retry the SAME call once if approved. Do not retry before a decision, do not route around the hold via c3_shell or another tool, and do not re-file — duplicates collapse into the pending request. If the S8 says no request could be filed, ask the user in chat.
12. **AGENT CONFIG** (v2.46.0+): `c3_artifacts(action='status|list|history|diff|restore')` — version history for the files that shape the agent itself: instruction docs (CLAUDE.md/AGENTS.md/GEMINI.md), settings/hooks, MCP configs, .claude skills/agents/commands. Out-of-band edits are captured automatically; `diff` any version against live, `restore` writes a prior version back (forward-only, ledger-logged). WRITES to these files PAUSE by default (v2.100.0+): .mcp.json, instruction docs, and .claude hook/skill/agent/command bodies are a builtin confirm tier — expect `[c3-access:confirm]` and follow the CONFIRM HOLDS flow above; reads stay open.
