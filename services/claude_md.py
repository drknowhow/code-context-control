...
9. **DELEGATE** (one tier down, same provider): `c3_delegate(task, task_type, context|file_path)` — default `backend='host'` runs your own provider at `tier='small'` (Claude Code: Haiku; `medium`=Sonnet). Use it for bounded work whose answer is a few lines: summarize a diff or log, explain a function, triage a traceback, draft a docstring or test cases. `scout=true` lets it Read/Grep/Glob the project itself for lookups. Multi-step work in Claude Code: the `c3-scout` (haiku, read-only) and `c3-worker` (sonnet) subagents. `c3_agent(workflow=...)` for multi-model pipelines
...
2. c3_read(file_path) maps a file (or directory); then c3_read(symbols=['Class.method']|lines=[a,b]) — BEFORE any file read
...
DO NOT: start with native Read/Grep/Glob/Edit, skip c3_validate, read full files without mapping them first."""
